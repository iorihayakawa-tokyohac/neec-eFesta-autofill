...
def wait_for_user_ack(prompt: str):
    message = prompt.strip() or "Press Enter to continue."
...
def _wait_manual_start(ctx, summary_page: Page | None, wait_for_viewer: float = 40.0) -> Page | None:
    ...
    def _wait_input():
        try:
            user_input["value"] = input().strip().lower()
        except Exception:
            user_input["value"] = ""
        finally:
            input_ready.set()
    ...
    while not input_ready.is_set():
        try:
            ...
            pass
        auto_viewer = poll_viewer_fast(ctx, duration_ms=600, interval_ms=200)
        if auto_viewer:
            logt("manual-wait: viewer auto-detected")
            input_ready.set()
            safe_focus(auto_viewer)
            SKIP_NEXT_COOP = True
            return auto_viewer
        time.sleep(0.4)

    value = user_input["value"] or ""
    if value == "q":
...
def run_two_pass_cycle(ctx, initial_viewer: Page) -> tuple[Page | None, Page | None]:
    ...
    if not answers:
        ...
        wait_for_user_ack("Answer list missing. Resolve manually, then press Enter to terminate this attempt.")
    ...
    if not viewer:
        ...
        wait_for_user_ack("Viewer not ready. Press Enter to abandon this session.")
        return _fail_and_cleanup(summary_page_fail)
    ...
    with sync_playwright() as p:
        ...
        while session_target is None or session_index <= session_target:
            ...
            if not summary_page or not last_viewer:
                ...
                wait_for_user_ack("Press Enter to exit. Review the viewer and restart when ready.")
                return
            ...
            if not viewer:
                ...
                wait_for_user_ack("Press Enter to exit the automation safely.")
                return
