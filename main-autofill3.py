...
def auto_restart_quiz(
    ...
    label = "2回目（満点）" if previous_viewer else "次のセッション"
    print("[restart] 自動再開は行いません。戻る→開始するを手動で行い、設問1が表示されたら Enter を押してください。")
...
def main():
    with sync_playwright() as p:
        ...
        while session_target is None or session_index <= session_target:
            ...
            time.sleep(2.0)
            viewer = _prompt_user_for_viewer(ctx, f"{session_index}回目（意図的ミス）")
            if not viewer:
                if SKIP_ON_FAIL:
