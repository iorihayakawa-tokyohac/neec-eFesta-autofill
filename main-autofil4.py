...
def auto_restart_quiz(
    ...
    if summary_page and _auto_click_start_from_page(summary_page):
        viewer = pick_viewer_page(ctx, timeout_s=wait_for_viewer, exclude_pages=exclude_pages)
        if viewer and _is_viewer_like(viewer):
            global SKIP_NEXT_COOP
            SKIP_NEXT_COOP = True
            return viewer
